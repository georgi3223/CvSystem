...
<div class="p-6 bg-white rounded shadow-md md:w-1/2 lg:w-1/3 m-auto">
 <h1 class="text-2xl font-semibold">Create CV</h1> @if(session('success')) <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mt-4">
  {{ session('success') }}
 </div> @endif <form x-data="cvForm()" x-init="init()" class="mt-4"> @csrf
 <div class="mb-4">
...
<div class="mb-4">
  ...
  <!-- ... Other input fields ... -->
     ...
     <div class="mb-4">
            ...
            <button type="button" @click="showUniversityPopup = true" class="mt-1 px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">
                Add New University
            </button>
        </div>
...
function cvForm() {
    return {
        ...
        newUniversityName: '',
        newTechnologyName: '',
        showUniversityPopup: fаlse,
        showTechnologyPopup: false,

        init() {
            // Initialize any necessary data or behaviors when the component is mounted
        },
        openUniversityPopup() {
        ...
        addUniversity() {
            if (this.newUniversityName.trim() !== '') {
                // Perform AJAX request to add the new university
